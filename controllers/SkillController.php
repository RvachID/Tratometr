...
final class SkillController extends Controller
{
    ...
    private function handleCommand(AliceListService $service, int $userId, string $command): string
    {
        ...
        if (preg_match('~(что в списке|что купить|список покупок|список)$~u', $command)) {
            $list = $service->getActiveList($userId);
            if (!$list) {
                return 'Список покупок пуст. Скажи: "добавь молоко" или "добавь хлеб и сахар", чтобы добавить продукт в список.';
            }
        ...
        // дефолт
        return 'Я могу вести список покупок. Скажи: "добавь молоко" или "добавь хлеб и сахар", чтобы добавить продукт в список или спроси "что в списке"';
    }
